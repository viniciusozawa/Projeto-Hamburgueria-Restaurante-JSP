...
<head>
    ...
    <title>Clientes - Painel do Gerente</title>
...
<div class="d-flex">
    ...
    <nav class="admin-sidebar" id="adminSidebar">
        ...
        <div class="sidebar-nav">
            ...
            <a href="${pageContext.request.contextPath}<%= URL_BASE %>/ClienteController?opcao=listar" class="sidebar-link active">
        ...
        <div class="admin-page">
            ...
            <div class="admin-card mb-4">
                <div class="admin-card-header">
                    <h5><i class="fa-solid fa-users"></i> ${empty opcao ? 'Novo Cliente' : 'Gerenciar Cliente'}</h5>
                </div>
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="nomeCliente"
                                       value="${nomeCliente}" placeholder="Nome completo" required />
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">CPF</label>
                                <input type="text" class="form-control" name="cpfCliente"
                                       value="${cpfCliente}" placeholder="000.000.000-00" />
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Telefone</label>
                                <input type="text" class="form-control" name="telefone"
                                       value="${telefone}" placeholder="(35) 99999-0000" />
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Senha</label>
                                <input type="password" class="form-control" name="senhaCliente"
                                       value="${senhaCliente}" placeholder="Senha de acesso" />
                            </div>
                            <div class="col-md-3">
                                <label class="form-label">Data Cadastro</label>
                                <input type="text" class="form-control" name="dataCadastro"
                                       value="${dataCadastro}" readonly />
                            </div>
                            <div class="col-12 d-flex gap-2 mt-1">
                                <button type="submit" class="btn-admin-salvar">
                                    <i class="fa-solid fa-floppy-disk"></i> Salvar
                                </button>
                                <a href="${pageContext.request.contextPath}<%= URL_BASE %>/ClienteController?opcao=cancelar"
            ...
            <c:if test="${not empty clientes}">
                <div class="admin-card">
                    <div class="admin-card-header">
                        <h5><i class="fa-solid fa-list"></i> Clientes Cadastrados</h5>
                    </div>
                            ...
                            <table class="admin-table">
                                ...
                                <tbody>
                                    <c:forEach var="item" items="${clientes}">
                                        <tr>
                                            <td class="text-muted">${item.codCliente}</td>
                                            <td><strong>${item.nomeCliente}</strong></td>
                                            <td>${item.cpfCliente}</td>
                                            <td>${item.telefone}</td>
                                            <td class="text-muted small">${item.dataCadastro}</td>
